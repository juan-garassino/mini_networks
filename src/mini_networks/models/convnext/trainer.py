...
class ConvNeXtTrainer(SupervisedTrainer):
    ...
    def _forward(self, model, batch, config: ConvNeXtConfig):
        images, labels = batch
        images = images.to(config.device)
        return model(images), labels

    def infer(self, config: BaseConfig, inputs: Any) -> Any:
        assert isinstance(config, ConvNeXtConfig)
...
def make_convnext_dataloader(config: ConvNeXtConfig, split: str = "train") -> DataLoader:
